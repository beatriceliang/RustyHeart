...
class RustyHeart:
	def __init__(self):
		...
		space = {"door":pygame.image.load("images/smallerfrobot.png"),"music": pygame.mixer.Sound('music/songs/DestinyDay.wav'),"background":pygame.image.load("images/space/space.png"),"metal":pygame.image.load("images/space/spacestep.png").convert_alpha(),"cardboard":pygame.image.load("images/space/moonrock.png").convert_alpha()}
	...
	def updateState(self):
		...
		self.drawBkg(self.background,self.rusty.rect)
			...
			if(item.type=="cardboard" or diffX!=0 or item.type=="heart" or item.collide):
				if item.rect.left <self.screensize[0] and item.rect.right >0:
					if not(item.type=="heart" and item.visible==True):
						self.drawBkg(self.background,item.rect)
		#moves object
		for item in self.objects:
	...
	def loadLevel(self):
		...
		self.background = levelStuff["background"]
		self.drawBkg(self.background)
		self.objects = []
		self.Spikes = []
		if(self.heart!=None):
	...
	def main(self):
		...
		winMusic = pygame.mixer.Sound('music/songs/HappyBee.wav')
		...
		while True:
			...
			if self.state == "play":
				...
				t = str((pygame.time.get_ticks()-self.startTime)/1000)
				timer = afont.render(t,True,(255,255,255))
				timerRect = timer.get_rect()
				self.drawBkg(self.background,timerRect.move(self.screensize[0] - timerRect.width-10,10))
				self.screen.blit(timer,(self.screensize[0] - timerRect.width-10,10))
						...
						if event.key == pygame.K_UP:
							if self.Door != None and self.Door.active and self.rusty.rect.centerx <= self.Door.rect.right and self.rusty.rect.centerx >= self.Door.rect.left and self.rusty.rect.centery >= self.Door.rect.top and self.rusty.rect.centery <= self.Door.rect.bottom:
								...
								if self.level >= len(self.levels)-1:
									...
									self.highscore = False
									self.time = (self.endTime - self.startTime)/1000
									fp = open("scores.txt", "r")
									self.score = int(fp.read())
									fp.close()
									if self.time < self.score:
