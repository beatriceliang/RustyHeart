...
class Box:
	...
	def pickUp(self):
		if(self.type=="cardboard"):
			...
			if((self.rusty.location[0]<self.location[0]+self.rect.width*1.1 and self.rusty.location[0]>self.location[0]-self.rect.width*1.1) and (self.rusty.location[1]<self.location[1]+self.rect.height*2 and self.rusty.location[1]>self.location[1]-self.rect.height*2)):
				self.state = 'held'
				self.rusty.box = self
